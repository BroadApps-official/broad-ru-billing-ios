...
BROAD_API_MODULE=BroadRUBillingUI bash "$module_root/Scripts/generate_public_api_report.sh"
...
docc_warnings="$(
    rg -- ': warning:' "$docc_log" \
        | rg -- '(^|/)Sources/BroadRUBilling(UI)?/' \
        || true
)"
if [[ -n "$docc_warnings" ]]; then
    printf 'BroadRUBilling DocC emitted warnings:\n%s\n' "$docc_warnings"
    exit 1
fi

xcodebuild -quiet -scheme BroadRUBillingUI -destination 'generic/platform=iOS Simulator' -derivedDataPath "$derived_data" CODE_SIGNING_ALLOWED=NO docbuild > "$module_root/.build/DocCUIBuild.log" 2>&1
if rg -- ': warning:' "$module_root/.build/DocCUIBuild.log" | rg -- '(^|/)Sources/BroadRUBillingUI/'; then exit 1; fi
